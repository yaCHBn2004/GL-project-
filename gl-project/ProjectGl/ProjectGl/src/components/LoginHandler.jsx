...
const LoginHandler = () => {
    ...
    useEffect(() => {
            ...
            try {
                ...
                const isClient = await verifyToken("http://localhost:8000/core/verify-client", "/search");
                const isAdmin = await verifyToken("http://localhost:8000/core/verify-Admin", "/dashboard");
                if(isAdmin){
                    navigate("/dashboard")
                }
                else if (isLawyer){
